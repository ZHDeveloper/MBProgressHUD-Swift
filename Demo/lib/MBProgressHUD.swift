...
public class MBProgressHUD: UIView {
    ...
    func show(_ animated: Bool) {
        ...
        if graceTime > 0.0 {
            let newGraceTimer: Timer = Timer(timeInterval: graceTime, target: self, selector: #selector(handleGraceTimer(_:)), userInfo: nil, repeats: false)
    ...
    func hide(_ animated: Bool) {
        ...
        if let showStarted = showStarted, minShowTime > 0.0 {
            ...
            guard interv >= minShowTime else {
                minShowTimer = Timer(timeInterval: minShowTime - interv, target: self, selector: #selector(handleMinShowTimer(_:)), userInfo: nil, repeats: false)
                return
            }
        }
    ...
    @objc fileprivate func handleGraceTimer(_ theTimer: Timer) {
...
//        isFinished = true
    ...
    fileprivate func registerForNotifications() {
        NotificationCenter.default.addObserver(self, selector: #selector(statusBarOrientationDidChange(_:)), name: NSNotification.Name.UIApplicationDidChangeStatusBarOrientation, object: nil)
    }
    ...
    override public func layoutSubviews() {
        ...
        // Position elements
        var yPos: CGFloat = round(((bounds.size.height - totalSize.height) / 2)) + margin + yOffset
    ...
    override public func draw(_ rect: CGRect) {
        ...
        let boxRect: CGRect = CGRect(x: round((allRect.size.width - size.width) / 2) + xOffset, y: round((allRect.size.height - size.height) / 2) + yOffset, width: size.width, height: size.height)
